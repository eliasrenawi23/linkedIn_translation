...
type EvaluationResponse = CandidateEvaluation & {
  source: string;
  pipeline: { structuredExtraction: boolean; githubRepositoriesAnalyzed: number; githubProjectsSelected: number };
};
    ...
    <main className="app-container app-main">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        ...
        <section className="min-w-0 space-y-5">
          ...
          {result && <>
            <section className="app-surface overflow-hidden"><div className="grid gap-5 p-6 sm:grid-cols-[10rem_1fr] sm:items-center"><div className="flex aspect-square items-center justify-center rounded-full bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-xl shadow-blue-200"><div className="text-center"><strong className="block text-4xl">{result.overallScore}</strong><span className="text-xs font-bold uppercase tracking-widest text-blue-100">out of 120</span></div></div><div><p className="page-kicker">Overall portfolio signal</p><h3 className="text-2xl font-bold text-slate-900">Evidence-based technical profile</h3><p className="mt-2 text-sm leading-relaxed text-slate-500">{result.githubEnriched ? `Structured resume evidence plus ${result.pipeline.githubProjectsSelected} qualifying GitHub project${result.pipeline.githubProjectsSelected === 1 ? "" : "s"}.` : "Structured resume evidence only."}</p><div className="mt-4 flex flex-wrap gap-2"><span className="rounded-full bg-blue-100 px-3 py-1.5 text-xs font-bold text-blue-800">Structured extraction</span>{result.githubEnriched && <span className="rounded-full bg-violet-100 px-3 py-1.5 text-xs font-bold text-violet-800">{result.pipeline.githubRepositoriesAnalyzed} repos analyzed</span>}<span className="rounded-full bg-emerald-100 px-3 py-1.5 text-xs font-bold text-emerald-800">+{result.bonusPoints.total} bonus</span><span className="rounded-full bg-rose-100 px-3 py-1.5 text-xs font-bold text-rose-800">−{result.deductions.total} deductions</span></div></div></div></section>
